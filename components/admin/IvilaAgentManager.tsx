...
type UserDoc = { id: string | number; name?: string; username?: string; phone?: string; role?: 'admin' | 'agent'; createdAt?: string }
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const response = await fetch('/api/users', {
        ...
        body: JSON.stringify({ name: name.trim(), username: normalizedPhone, phone: normalizedPhone, password, role: 'agent' }),
      })
      const data = await response.json().catch(() => null) as any
    ...
    <div className={styles.simpleAdminGrid}>
      <section className={styles.formCard}>
        ...
        <form className={styles.fieldsGrid} onSubmit={submit}>
          ...
          <label className={`${styles.field} ${styles.span2}`}><span>شماره موبایل ورود</span><input required type="tel" inputMode="numeric" dir="ltr" placeholder="09121234567" value={phone} onChange={(e)=>setPhone(e.target.value)}/></label>
      ...
      <section className={styles.formCard}>
        ...
        <div className={styles.teamList}>{users.map((user)=><article key={user.id} className={styles.teamItem}><div className={styles.avatar}><UserRound size={17}/></div><div><strong>{user.name || user.phone || 'کاربر ivila'}</strong><span dir="ltr">{user.phone || user.username || 'شماره ثبت نشده'}</span></div><em className={user.role === 'admin' ? styles.adminRole : styles.agentRole}>{user.role === 'admin' ? <><ShieldCheck size={13}/> ادمین اصلی</> : 'مشاور'}</em></article>)}</div>
      </section>
    </div>
  </main>
}
